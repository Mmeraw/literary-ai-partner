    ...
    from: (_table: string) => ({
      select: (fields: string) => ({
        ...
        limit: async (_count: number) => ({
          data: [{ id: jobRow.id, validity_status: null }],
          error: null,
        }),
      }),
      update: (payload: Record<string, unknown>) => {
...
describe("Phase 2.4.c \u2014 apply-path failure classification proof", () => {
  ...
  test.each(APPLY_FAILURE_CASES)(
    ...
    async ({ code, messageFragment }) => {
      ...
      const updatePayload =
        harness.updatePayloads[harness.updatePayloads.length - 1];
      expect(updatePayload.last_error).toBe(envelope.message);
      expect(updatePayload.failure_envelope).toBeTruthy();
      const persistedEnvelope = updatePayload.failure_envelope as Record<
        string,
        unknown
      >;
      ...
      const persistedContext = persistedEnvelope.context as Record<
        string,
        unknown
      >;
      ...
      if (code === RevisionFailureCode.PARSE_ERROR) {
        ...
        expect(updatePayload.next_attempt_at).toBe(
