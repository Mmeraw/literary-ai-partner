# PR #293 — Seed-Band Divergence/Compression Observation Governance (Telemetry-First)

## Status

Lane opens after #292 SIPOC merge. This phase is observation-first and calibration-building only.

## Purpose

Replace the existing binary `Pass3Guard:DIVERGENCE_COLLAPSE_WARNING` with seed-band observational governance using `representation_compression_ratio`, while explicitly avoiding fail-closed behavior until empirical production distributions are established.

## Governance correction (binding for Phase 1)

Do **not** introduce `hard_fail` behavior yet for `representation_compression_ratio`.

Phase 1 remains telemetry-first observation with states:
- `pass`
- `warn`
- `observe`

Goal of #293: replace noisy binary divergence warning with bounded observational governance, not immediate enforcement escalation.
...
With SIPOC now live, we can observe and characterize real distributions first, then consider fail-closed governance in a later lane.

## Scope (in)

1. Replace binary divergence-collapse warning with seed-band ratio observation logic.
...
- [ ] Seed-band pass/warn/observe logic implemented in runtime quality guard path.
...
- `lib/evaluation/governance/runtimeQualityGuards.ts` — replace binary warning with seed-band observational bands
...
Initial thresholds are provisional and must be treated as observational heuristics.
...
- Establishes evidence-driven governance progression (measure -> observe -> calibrate -> enforce).
