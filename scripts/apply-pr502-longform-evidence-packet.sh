#!/usr/bin/env bash
# Patch script for PR #502 — fix/pass4-longform-evidence-packet
#
# Goal:
#   Fix Pass 4 Perplexity adjudication for long-form manuscripts by
#   replacing the first-3000-character excerpt with a bounded
#   representative long-form evidence packet (opening / early /
#   middle / late / ending windows, ending always included).
#
# Branch base: PR #501 HEAD c65d3c13
#
# What this script does (in order):
#   1. Refuses to run if not on a clean worktree.
#   2. Creates the new branch fix/pass4-longform-evidence-packet from
#      c65d3c13 if it doesn't already exist, and checks it out.
#   3. Writes lib/evaluation/pipeline/pass4EvidencePacket.ts (new file).
#   4. Writes tests/evaluation/pipeline/pass4EvidencePacket.test.ts (new file).
#   5. Patches lib/evaluation/pipeline/perplexityCrossCheck.ts with:
#        - new import line for buildPass4EvidencePacket
#        - replacement of `manuscriptExcerpt.slice(0, 3000)` prompt
#          block with the labeled "REPRESENTATIVE MANUSCRIPT EVIDENCE
#          PACKET" block
#        - extra packet-metadata fields on the pass4_start emit
#   6. Runs the Pass 4 unit suites + the new packet suite.
#
# It does NOT:
#   - touch runPipeline.ts (already passes opts.manuscriptText through)
#   - touch Supabase or job lifecycle
#   - commit any manuscript fixtures
#   - change PASS4_WEAK_AGREEMENT governance
#   - flip the adjudication mode to advisory
#   - commit or push (you review `git diff` then decide)
#
# Run from the repo root:
#   cd /workspaces/literary-ai-partner
#   git fetch origin
#   bash /path/to/apply-pr502-longform-evidence-packet.sh
#
set -euo pipefail
...
BASE_SHA="c65d3c13"
TARGET_BRANCH="fix/pass4-longform-evidence-packet"

# Verify the base SHA exists locally (fetch if not).
if ! git cat-file -e "${BASE_SHA}^{commit}" 2>/dev/null; then
  echo "Fetching origin to obtain base SHA ${BASE_SHA}..."
  git fetch origin
fi
if ! git cat-file -e "${BASE_SHA}^{commit}" 2>/dev/null; then
  echo "ERROR: base SHA ${BASE_SHA} not found even after fetch." >&2
  exit 1
fi

# Create or check out the new branch.
if git show-ref --quiet "refs/heads/${TARGET_BRANCH}"; then
  echo "Branch ${TARGET_BRANCH} already exists locally; checking out."
  git checkout "${TARGET_BRANCH}"
else
  echo "Creating branch ${TARGET_BRANCH} from ${BASE_SHA}..."
...
# --- Write pass4EvidencePacket.ts ------------------------------------------

mkdir -p "$(dirname "$PACKET")"
cat > "$PACKET" <<'PACKET_EOF'
/**
...
export function buildPass4EvidencePacket(
  ...
  if (sourceWords < opts.longFormWordThreshold) {
    const capped = cleaned.slice(0, Math.min(18_000, opts.hardCapChars));
    const text = capped;
    return {
      text,
      sourceWords,
      ...
      includesEnding: text.length >= sourceChars,
    };
  }
  ...
  // Assemble text with labeled headers.
  const labeled = slices
    .filter((s) => s.body.length > 0)
    .map(
      (s) =>
        `--- WINDOW: ${s.label.toUpperCase()} ` +
        `(~${s.body.length} chars) ---\n${s.body}`,
    )
    .join("\n\n");

  // Final hard cap enforcement (defensive — should already fit).
  const finalText =
    labeled.length > opts.hardCapChars
      ? labeled.slice(0, opts.hardCapChars)
      : labeled;

  const selectedWindows = slices
    .filter((s) => s.body.length > 0)
    .map((s) => s.label);
...
# --- Write pass4EvidencePacket.test.ts -------------------------------------

mkdir -p "$(dirname "$PACKET_TEST")"
cat > "$PACKET_TEST" <<'PACKET_TEST_EOF'
...
# --- Patch perplexityCrossCheck.ts via Python (exact verbatim) -------------
...
# 1. Add the import for buildPass4EvidencePacket right after the
#    perplexityCrossCheckRequest import block.
import_old = '''import {
...
assert src.count(import_old) == 1, "Patch 1: import anchor not unique or missing"
src = src.replace(import_old, import_new, 1)

# 2. Replace the "MANUSCRIPT EXCERPT (first 3000 chars)" prompt block
#    with the labeled "REPRESENTATIVE MANUSCRIPT EVIDENCE PACKET" block.
excerpt_old = '''MANUSCRIPT EXCERPT (first 3000 chars):
${manuscriptExcerpt.slice(0, 3000)}'''
excerpt_new = '''REPRESENTATIVE MANUSCRIPT EVIDENCE PACKET:
${evidencePacket.text}'''
assert src.count(excerpt_old) == 1, "Patch 2: excerpt anchor not unique or missing"
src = src.replace(excerpt_old, excerpt_new, 1)

# 3. Build the packet just before the prompt assembly. Insert it right
#    after the criteriaBlock construction. We anchor on the closing
#    `).join("\n");` line of the CRITERIA_KEYS.map block.
build_old = '''  const criteriaBlock = CRITERIA_KEYS.map((key) => {
...
assert src.count(build_old) == 1, "Patch 3: criteriaBlock anchor not unique or missing"
src = src.replace(build_old, build_new, 1)

# 4. Add packet metadata to the pass4_start emit.
log_old = '''  emitPass4Log("log", {
    event: "pass4_start",
    job_id: logJobId,
...
assert src.count(log_old) == 1, "Patch 4: pass4_start anchor not unique or missing"
src = src.replace(log_old, log_new, 1)

p.write_text(src)
print("[apply-pr502] perplexityCrossCheck.ts patched cleanly.")
PY_EOF

# --- Grep guards -----------------------------------------------------------

echo ""
echo "=== Grep guard: legacy slice(0, 3000) must be gone ==="
if grep -n "manuscriptExcerpt.slice(0, 3000)" "$CROSS"; then
  echo "ERROR: legacy excerpt slice still present!" >&2
  exit 1
else
  echo "OK: legacy slice(0, 3000) removed."
fi

echo ""
echo "=== Grep guard: helper import is present ==="
grep -n 'buildPass4EvidencePacket' "$CROSS" || {
  echo "ERROR: helper import missing!" >&2
  exit 1
}
...
echo ""
echo "=== Running new packet test suite ==="
...
echo ""
echo "============================================================"
echo "PR #502 patch applied. Nothing committed."
echo "Next steps:"
echo "  1. Review:  git diff"
echo "  2. Run the Froggin local scenario internally."
echo "  3. Inspect Pass 4 logs for pass4_packet_chars,"
echo "     pass4_selected_windows, pass4_includes_ending."
echo "  4. If green, commit and push to open PR #502:"
echo ""
...
echo "     git commit -m 'fix(pass4): representative long-form evidence packet for adjudication'"
