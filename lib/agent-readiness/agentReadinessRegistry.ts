...
export const AGENT_READINESS_PROCESS_REGISTRY: readonly AgentReadinessProcessEntry[] = [
  ...
  },
  {
    ...
    backwardKick: 'none — KNOWN GAP: DB failure is currently non-fatal (see notes)',
    dirtyDataRules: [
      'Never write a section with status=approved at generation time — approval is an explicit author action.',
      'KNOWN GAP: DB write failure (saveError) is currently treated as non-fatal. The generate route logs a warning and returns 200 with the generated content anyway (generate/route.ts L526: "Non-fatal — still return the generated content"). The AI_GOVERNANCE contract requires DB errors to surface as 500. This is an explicit gap requiring a future fix.',
    ],
    ...
    notes: 'KNOWN GAP: DB persistence failure is non-fatal at runtime. generate/route.ts catches saveError, logs a warning, and still returns 200 + content. This violates the AI_GOVERNANCE requirement that system errors must not be masked. Upsert conflict key: (user_id, manuscript_id, section_type). Regeneration overwrites existing draft.'
  },
  {
    sequence: 5,
    ...
    dirtyDataRules: [
      ...
      'KNOWN GAP: Section approval is currently UI-only client state (sectionStates in AgentReadinessClient.tsx). The Approve button does not call an API or write status=approved to agent_readiness_sections. Approval persistence to DB is a planned required feature.',
    ],
    ...
    certificationStatus: 'missing_critical',
    fitGapStatus: 'critical',
    notes: 'KNOWN GAP: The Approve button in AgentReadinessClient.tsx is a disabled stub with no onClick handler. Approval is tracked only in React state (sectionStates). No API endpoint writes status=approved to DB. The getPackageStatuses() DB check therefore always returns Not Started or Draft, never Approved. This is the highest-priority seam for the next AR sprint.',
  },
  {
    sequence: 6,
    ...
    processContract: 'Design intent: all 6 canonical sections must have status=approved before packageStatus transitions to Approved. ACTUAL BEHAVIOR: The DB check (getPackageStatuses) can never return Approved because no API writes status=approved to agent_readiness_sections. The client tracks approvedCount via sectionStates React state only. The download CTA is gated on allSectionsStarted (all 6 sections have been generated, regardless of approval), not on allSectionsApproved.',
    ...
    forwardKick: 'AR08_EXPORT (when all 6 started; approval enforcement is a planned required gate)',
    backwardKick: 'AR05_AUTHOR_REVIEW (when any section not yet generated)',
    dirtyDataRules: [
      'KNOWN GAP: allSectionsApproved is based on client state only, not DB state.',
      'KNOWN GAP: Download CTA is gated on allSectionsStarted, not allSectionsApproved.',
      'packageStatus Approved cannot be set by getPackageStatuses since no approval API writes to DB.',
    ],
    ...
    certificationStatus: 'missing_critical',
    fitGapStatus: 'critical',
    notes: 'KNOWN GAP: Completeness check is designed around approved status but approval is not persisted. The actual CTA gate is allSectionsStarted (every section has a draft). Full approval enforcement requires: (1) section approval API, (2) client calling it on Approve click, (3) DB read on page load.',
  },
  {
    sequence: 7,
  ...
  },
  {
    ...
    processContract: 'Assemble and export package in one step. The download route takes manuscriptTitle, format, and sections inline from the client (not from DB). ACTUAL GATE: requires manuscriptTitle + at least one section (Object.keys(sections).length > 0). It does NOT require all 6 sections, does NOT check section approval status, and does NOT read from the DB. Missing sections are silently skipped in canonical order. Canonical section order in output: query_pitch → query_letter → what_makes_unique → synopsis → comparables → author_bio. Output format: .txt (plain text with dividers) or .docx (Word 2003 XML). Export must not imply agent interest, representation, publication, sales, or market outcome. NOTE: this is both the assembly and export step — there is no separate DB-persisted package assembly record.',
...
export const AGENT_READINESS_ARTIFACT_REGISTRY: readonly AgentReadinessArtifactEntry[] = [
  ...
  },
  {
    ...
    dirtyDataRule: 'KNOWN GAP: DB write failure is non-fatal \u2014 content may be returned to author without being persisted. status must not be set to approved at generation time.',
  ...
  },
  {
    ...
    dirtyDataRule: 'KNOWN GAP: decision is stored in client React state only (sectionStates). No API persists approval decision to DB. Author bio decision must not override user-supplied facts with invented content.',
  ...
  },
  {
    ...
    completenessMetric: 'approvedCount integer from client state; allSectionsStarted boolean from client state',
    accuracyMetric: 'KNOWN GAP: allSectionsApproved is derived from client state only; DB cannot confirm Approved because no approval API exists.',
    dirtyDataRule: 'Must not assert allSectionsApproved=true with approvedCount < 6. KNOWN GAP: download CTA is gated on allSectionsStarted, not allSectionsApproved.',
...
export const AGENT_READINESS_AUTHORITY_SOURCE_REGISTRY: readonly AgentReadinessAuthoritySourceEntry[] = [
  ...
  },
  {
    ...
    appliesToStageIds: ['AR01_MANUSCRIPT_ELIGIBILITY', 'AR02_SECTION_GENERATION', 'AR03_QUALITY_GATE', 'AR04_SECTION_PERSISTENCE', 'AR05_AUTHOR_REVIEW', 'AR06_COMPLETENESS_CHECK', 'AR07_BATCH_GENERATION', 'AR08_EXPORT', 'AR09_HISTORY'],
    appliesToArtifacts: ['manuscript_context_v1', 'section_generation_request_v1', 'section_generation_result_v1', 'quality_gate_result_v1', 'agent_readiness_section_v1', 'author_review_decision_v1', 'package_completeness_result_v1', 'agent_readiness_package_v1', 'package_export_v1'],
