...
const AGENT_READINESS_PROCESS_REGISTRY_SOURCE: readonly AgentReadinessProcessEntrySource[] = [
  ...
  },
  {
    ...
    processContract: 'Call OpenAI GPT-4o with certified evaluation/revise context and gold standard prompts. Agent Readiness may package certified artifacts but must not reinterpret UED, Revise ledger, scores, recommendations, or business rules. Apply the DETERMINISTIC three-part length policy from lib/config/lengthPolicy.ts (MIN floor / BASE target / hard CAP = base + fixed integer overage) — NO percentage tolerance bands anywhere; the LLM never computes a percentage. Mode: generate (fresh) | regenerate (discard prior) | improve (refine existing). Section word bounds (min/base/cap): query_letter 200/450/500, what_makes_unique 60/150/170, synopsis short 100/150/180, synopsis standard 250/450/500, synopsis extended 500/750/1000, query_pitch 25/50/75, comparables 60/200/225, author_bio 50/200/225. Author bio requires authorBioInput; system must shape only supplied facts — never invent credentials.',
  ...
  },
  {
    ...
    processContract: 'Reject output if: text < 20 chars; contains editorial meta-language patterns; contains unresolved placeholder patterns ([Author Name], [Title], etc.); word count > section CAP (base + fixed integer overage, NO percentage); word count < section MIN floor (INSUFFICIENT_EXPLANATION — users are guaranteed a minimum amount of explanation, so a thinner answer is kicked back for regeneration, never padded/fabricated). Any over-CAP author-facing prose that is trimmed is trimmed at a COMPLETE-SENTENCE boundary only (NO_MIDSENTENCE_TRUNCATION) — never mid-sentence, never mid-word. Pass criteria must all hold simultaneously.',
...
export const AGENT_READINESS_KICK_MATRIX: readonly AgentReadinessKickEntry[] = [
  ...
  },
  {
    ...
    detectedAt: 'AR03_QUALITY_GATE',
    description: 'Generated word count exceeds the section hard CAP (base + fixed integer overage) from lib/config/lengthPolicy.ts. No percentage tolerance.',
    blocking: true,
    blocksPackageAssembly: false,
    remediation: 'Retry generation with tighter constraint. Deterministic per-section word CAPs (lib/config/lengthPolicy.ts): query_letter 500, what_makes_unique 170, synopsis short 180 / standard 500 / extended 1000, query_pitch 75, comparables 225, author_bio 225.',
    httpStatus: 422,
  },
  {
    kickCode: 'OUTPUT_TOO_THIN',
    detectedAt: 'AR03_QUALITY_GATE',
    description: 'Generated word count is below the section MIN floor from lib/config/lengthPolicy.ts (query_letter <200, what_makes_unique <60, synopsis short <100 / standard <250 / extended <500, query_pitch <25, comparables <60, author_bio <50). Kick back for regeneration — never pad or fabricate.',
    blocking: true,
    blocksPackageAssembly: false,
