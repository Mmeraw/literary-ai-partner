...
export function getSupabaseClient() {
  if (!supabaseClient) {
    // During Next.js build, env vars may be missing - return null to avoid build failures
    if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
      if (process.env.NODE_ENV === "production" && typeof window === "undefined") {
        // Server-side during build - return null, route handlers must check
        return null;
      }
      // Development or client-side - still try to create client for better error messages
    }
...
export function getSupabaseAdminClient() {
  if (!supabaseAdminClient) {
    if (!SUPABASE_SERVICE_ROLE_KEY) {
      throw new Error(
        "[RG-SUPABASE] SUPABASE_SERVICE_ROLE_KEY is required for admin client in tests/jobs"
      );
    }
