...
describe("resolveEvaluationRuntimeConfig", () => {
  it("resolves deterministic defaults", () => {
    ...
    expect(config.timeouts.passTimeout.valueMs).toBe(360000);
    expect(config.timeouts.openAiTimeout.valueMs).toBe(360000);
  });
  ...
  it("throws when worker max execution exceeds the lease policy ceiling", () => {
    ...
    ).toThrow(/EVAL_WORKER_MAX_EXECUTION_MS must be between 10000 and 360000/);
  });

  it("accepts explicit 360000ms worker max execution when lease is aligned", () => {
    const config = resolveWithoutBaseline({
      EVAL_WORKER_LEASE_MS: "360000",
      EVAL_WORKER_MAX_EXECUTION_MS: "360000",
    });

    expect(config.worker.leaseMs).toBe(360000);
    expect(config.worker.maxExecutionMs).toBe(360000);
  });
