...
import type { MaterializableOpportunity, MaterializableDecision } from "@/lib/revision/finalReviewTextApplication";
...
type RuntimeQueueOpportunity = MaterializableOpportunity;
type RuntimeDecision = MaterializableDecision;
...
async function loadRuntimeContext(input: FinalReviewRuntimeInput): Promise<RuntimeContext> {
  ...
  const queueByOpportunityId = new Map(
    queuePayload.opportunities.map((opportunity) => [
      opportunity.id,
      toMaterializableOpportunity(opportunity),
    ]),
  );
...
function applyTextSnapshots(ctx: RuntimeContext): { text: string; applied: RuntimeDecision[]; blocked: string[] } {
  ...
  if (blocked.length > 0) {
    return { text: scrubInternalReportLeakage(ctx.sourceText), applied: [], blocked };
  }

  const result = applyValidatedFinalReviewDecisions({
    sourceText: ctx.sourceText,
    decisions: ctx.decisions,
    opportunitiesById: ctx.queueByOpportunityId,
  });
  ...
  if (result.rejectedDecisions.length > 0) {
    return {
      text: result.text,
      applied: [],
      blocked: [...blocked, ...result.rejectedDecisions.map((rejection) => rejection.reason)],
    };
  }

  const applied = result.appliedDecisionIds
    .map((id) => ctx.decisions.find((decision) => decision.id === id))
    .filter((decision): decision is RuntimeDecision => Boolean(decision));

  return { text: result.text, applied, blocked };
}
