...
const REVISE_PROCESS_REGISTRY_SOURCE: readonly ReviseStageSource[] = [
  {
    ...
    inputRequiredFields: ['criteria', 'confirmed_mode', 'enrichment', 'overview', 'dcip_compliance'],
    inputMetrics: ['evaluation_result_v2 present', 'author_exposure_certification_v1 dcip_compliance passes', 'required constitutional authorities loaded', 'no blocking audit verdicts', 'criteria count >= 1'],
    ...
    processContract: 'Build evidence-backed revision opportunities from certified evaluation lineage without re-running evaluation and without consuming renderer/download output. Every opportunity must trace to an evaluation finding or story ledger entry.',
    ...
    outputMetrics: ['opportunity count is evidence-driven and may be zero', 'no Ready opportunity without evidence_anchor or manuscript_wide_support', 'all criteria traceable', 'DCIP compliance and constitutional authority status emitted in quality_manifest'],
  ...
  },
  {
    ...
    outputMetrics: ['anchor resolved in source', 'all six diagnostic fields populated', 'Ready items expose exactly 3 valid A/B/C candidates; Needs Targeting exposes no author-acceptance prose'],
