...
export const REVISE_PROCESS_REGISTRY: readonly ReviseStage[] = [
  ...
  },
  {
    ...
    inputRequiredFields: ['opportunity_id', 'severity', 'revision_operation', 'evidence_anchor', 'issueStatement', 'symptom', 'cause', 'fixDirection', 'readerEffect'],
    ...
    notes: 'admission_status is exactly: admission_passed | withheld. Six-part diagnostic (symptom, cause, fixDirection, readerEffect, operationNote, evidence) required for admission.',
  },
  {
    sequence: 30,
  ...
  },
  {
    ...
    codeSurfaces: ['planned: lib/revision/completionCertification.ts', 'lib/revision/engine.ts'],
    processContract: 'Certify revision completion when all ready_for_revise items have persisted author decisions. Produce revision_completion_record_v1. Completion does not require all needs_targeting items to be decided.',
    ...
    notes: 'Planned required gate. No formal completion certification currently exists; the engine.ts has partial completion tracking. completion_type will be: full | partial | needs_targeting_deferred.',
  },
  {
    sequence: 85,
    phase: 'Cross-Check',
...
export const REVISE_ARTIFACT_REGISTRY: readonly ReviseArtifact[] = [
  ...
  },
  {
    ...
    requiredFields: ['opportunity_id', 'readiness', 'revisionOperation', 'candidateTexts', 'sourceText', 'sourceLocationLabel', 'issueStatement', 'symptom', 'cause', 'fixStrategy', 'readerImpact'],
...
export const REVISE_FIELD_REGISTRY: readonly ReviseField[] = [
  ...
  { field: 'completion_type', canonicalType: 'enum', ownerArtifact: 'revision_completion_record_v1', allowedValues: 'full | partial | needs_targeting_deferred', validationRule: 'Planned field. full = all ready items decided. partial = some decided, some deferred. needs_targeting_deferred = all ready items decided; needs_targeting items explicitly deferred.', fitGapStatus: 'critical' },
