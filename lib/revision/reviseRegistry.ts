...
export const REVISE_AUTHORITY_SOURCE_REGISTRY: readonly ReviseAuthoritySource[] = [
  ...
  },
  {
    ...
    executionUse: 'RevisionSessionStatus allowed transitions: open→findings_ready→synthesis_started→proposals_ready→applied|failed|failed_retryable. Illegal transitions throw; they do not write.',
    notes: 'applied and failed are terminal states. failed_retryable allows re-entry to open/findings_ready/synthesis_started. Recovery path is executable via REVISE_KICK_MATRIX.',
  },
  {
...
const REVISE_PROCESS_REGISTRY_SOURCE: readonly ReviseStageSource[] = [
  {
    ...
    activeState: 'active',
    supplier: 'Phase 5 certified evaluation_result_v2 plus author exposure certification, constitutional authority status, story ledger, WAVE plan, and diagnostic findings',
    ...
    inputRequiredFields: ['criteria', 'confirmed_mode', 'enrichment', 'overview', 'dcip_compliance', 'canonicalOpportunityLedger.opportunities', 'canonicalOpportunityLedger.recommendation_dispositions', 'canonicalOpportunityLedger.forensics_contract_version'],
    inputMetrics: ['evaluation_result_v2 present', 'author_exposure_certification_v1 dcip_compliance passes', 'required constitutional authorities loaded', 'no blocking audit verdicts', 'recommendation lineage coverage = 100%', 'admitted_authority_coverage_ratio = 1', 'suppression forensics reconcile to persisted dispositions', 'canonical authority present even when opportunity count = 0'],
    codeSurfaces: ['lib/revision/opportunityLedger.ts', 'lib/revision/revisionOpportunityLedgerContract.ts', 'lib/revision/normalizeFindings.ts'],
    processContract: 'Project only admitted canonical opportunities from certified evaluation lineage without re-running evaluation and without consuming renderer/download output, report summaries, scores, or dispositions as queue authority. Suppressed and informational recommendations retain lineage without becoming work. Every ledger opportunity must trace to an admitted disposition and evaluation finding or story ledger entry.',
    outputArtifacts: ['revision_opportunity_ledger_v1'],
    outputRequiredFields: ['quality_manifest', 'quality_manifest.dcip_compliance', 'quality_manifest.constitutional_authority_registry', 'opportunities[].opportunity_id', 'opportunities[].finding_id', 'opportunities[].criterion', 'opportunities[].severity', 'opportunities[].evidence_anchor', 'opportunities[].manuscript_coordinates', 'opportunities[].revision_operation', 'opportunities[].preflight_status', 'opportunities[].grounding_status'],
    outputMetrics: ['opportunity count equals admitted disposition count and may be zero', 'suppressed/informational queue identity count = 0', 'duplicate canonical opportunity count = 0', 'no Ready opportunity without evidence_anchor or manuscript_wide_support', 'all ledger opportunities traceable', 'DCIP compliance and constitutional authority status emitted in quality_manifest'],
    forwardKick: 'RS02_QUEUE_ADMISSION',
    backwardKick: 'S10b_PHASE5_AUTHOR_EXPOSURE_GATE if evaluation_result_v2 is absent/uncertified, canonical authority is missing/malformed, or recommendation disposition lineage is incomplete',
    dirtyDataRules: ['canonical opportunity authority missing or malformed', 'unknown or inconsistent suppression-forensics contract', 'ledger opportunity lacks admitted disposition', 'admitted disposition lacks ledger opportunity', 'suppressed or informational disposition carries queue identity', 'missing evidence_anchor without manuscript_wide_support', 'opportunity with no criterion', 'duplicate opportunity_id', 'missing/failing DCIP compliance', 'missing required constitutional authority'],
    retryBudget: 1,
    ...
    notes: 'opportunityLedger.ts is the single active producer and revisionOpportunityLedgerContract.ts is the shared persistence, reload, and Held Recovery validator. Revise consumes certified evaluation lineage and revision ledger provenance only — never Web/PDF/DOCX/TXT renderer output and never evaluation_report_view_model_v1 as revision authority. Hard caps only: short-form max 50, long-form max 100 opportunities. Caps are not targets. Empty opportunities remain valid governed authority. Context quality now correctly resolves repair_required as limited (not blocked); hydration eligibility accepts limited_context preflight status. Ledger quality manifest emits DCIP compliance and constitutional authority registry status inherited from certified Evaluation context.',
  },
  {
    sequence: 20,
    ...
    inputMetrics: ['all six diagnostic fields populated', 'revision_operation set', 'readiness classified', 'no renderer/download output used as admission authority'],
    ...
    processContract: 'Validate each opportunity against the Revise Card Contract. Items passing all gates become ready_for_revise; items failing any gate become needs_targeting. No item is deleted — withheld items persist for author targeting. Admission may use certified Evaluation lineage and ledger provenance only; renderer/download output invalidates the handoff.',
    ...
    backwardKick: 'RS01_LEDGER_ASSEMBLY if admitted-disposition ledger coverage < 100% or any queue item lacks canonical ledger backing',
    dirtyDataRules: ['queue item lacks canonical ledger opportunity', 'suppressed/informational recommendation entered admission', 'missing six-part diagnostic', 'candidate_text empty when operation requires one', 'anchor missing without manuscript_wide_support', 'canon gate fail', 'voice gate fail', 'renderer/download output used for Revise admission'],
    retryBudget: 0,
    ...
    certificationStatus: 'partial',
    fitGapStatus: 'emerging',
    notes: 'admission_status is exactly: admission_passed | withheld. Six-part diagnostic is exactly: symptom, cause, fixDirection, readerEffect, evidence_anchor, revision_operation. UI/card aliases: fixStrategy=fixDirection, readerImpact=readerEffect, operationTargeting=revision_operation + location, evidence.quotedExcerpt=evidence_anchor. Admission gate now accepts limited_context preflight and limited context_quality (not just passed/clean). If Revise admission depends on renderer/download output, the handoff is invalid and must kick back to the Evaluation authority boundary rather than proceed through Revise.',
  },
  {
    sequence: 30,
    ...
    processContract: 'Sort admitted opportunities by deterministic severity ranking (must → should → could), apply hard cap, and assemble author-facing revise queue. needs_targeting items appear in a separate advisory view. Queue owns lifecycle, admission, retry, lease, and presentation state only; it must not mutate certified UED, ViewModel, renderer, or download artifacts.',
    ...
    backwardKick: 'RS02_QUEUE_ADMISSION only if authorized input opportunities were lost; a governed zero-opportunity ledger is a valid empty queue',
    dirtyDataRules: ['queue item count differs from admitted/withheld authorized input count', 'queue_length > hard_cap after trim', 'gaps in display_index', 'duplicate opportunity_id in queue'],
    retryBudget: 0,
    ...
    fitGapStatus: 'emerging',
    notes: 'WorkbenchSource order: evaluation > deep_revision > baseline_discovery > surface_polish. Reset queue API exists at app/api/revise/reset-queue/route.ts. Queue state is presentation/lifecycle state; certified Evaluation artifacts are immutable inputs.',
  },
  {
    sequence: 40,
    phase: 'Workbench',
    ...
    processContract: 'Load and hydrate one workbench opportunity: resolve anchor in source manuscript, hydrate diagnostic fields, load A/B/C candidates, resolve mode contract, and surface evidence for author review. Workbench reads ledger provenance; it must not consume renderer/download output or mutate certified UED/VM artifacts.',
    ...
    outputMetrics: ['anchor resolved in source', 'all six diagnostic fields populated', 'Ready items expose exactly 3 valid A/B/C candidates; Needs Targeting exposes no author-acceptance prose', 'raw diagnostic leakage count = 0', 'each raw diagnostic input classified into one family', 'one author-safe explanation per distinct diagnostic family', 'author-safe context count <= distinct diagnostic family count'],
    ...
    dirtyDataRules: ['raw internal diagnostic code appears in author-facing output', 'diagnostic input disappears before family classification', 'classified family lacks author-safe explanation', 'duplicate author-safe explanation', 'anchor not found in source manuscript', 'diagnostic field empty or placeholder', 'Ready item missing exact A/B/C prose', 'generic or malformed candidate prose', 'mode contract missing'],
    retryBudget: 1,
    ...
    certificationStatus: 'emerging',
    fitGapStatus: 'emerging',
    notes: 'WorkbenchMode is exactly: direct-rewrite | repair-brief. WorkbenchScope: Line | Passage | Scene | Chapter | Structural | Manuscript. Workbench owns review presentation state only; persistence ownership begins at author decision/ledger sync.',
  },
  {
    sequence: 50,
    phase: 'Workbench',
    ...
    outputMetrics: ['exactly 3 candidates (A/B/C) unless mode restricts', 'no candidate duplicates original text', 'voice gate passes', 'A/B/C are materially distinct — no pair shares ≥ 0.80 content-token overlap and none are byte-identical (candidate_option_distinctness)', 'A/B/C SHAPE is non-empty — every option carries real revised prose ≥ 4 content tokens (candidate_option_shape)', 'A/B/C are evidence-grounded — every option overlaps the manuscript anchor prose or its rationale (candidate_option_evidence_grounding)'],
    forwardKick: 'RS09_CROSSCHECK_VERIFICATION (async, feature-flagged), then RS06_AUTHOR_DECISION',
    backwardKick: 'RS04_WORKBENCH_LOAD if generation fails; REVISE_ABC_NOT_DISTINCT kicks back to RS04_WORKBENCH_LOAD to regenerate a distinct, grounded, non-empty set',
    dirtyDataRules: ['candidate_text = original_text', 'empty candidate_text', 'voice gate violation', 'canon gate violation', 'missing mechanism or rationale', 'duplicate or near-duplicate A/B/C options (no real choice)', 'empty or hollow A/B/C option (SHAPE failure)', 'A/B/C option not grounded in manuscript/chapter/story evidence'],
    retryBudget: 1,
    failureDefinitions: getFailureRecoveryDefinitionsForCodes(['CANDIDATE_GENERATION_FAILED', 'CANDIDATE_VOICE_GATE_FAIL', 'CANDIDATE_CANON_GATE_FAIL', 'CANDIDATE_EMPTY', 'CANDIDATE_DUPLICATES_ORIGINAL', 'REVISE_ABC_NOT_DISTINCT']),
...
export const REVISE_ARTIFACT_REGISTRY: readonly ReviseArtifact[] = [
  {
    ...
    requiredFields: ['$.quality_manifest', '$.quality_manifest.dcip_compliance', '$.quality_manifest.constitutional_authority_registry', '$.opportunities[].opportunity_id', '$.opportunities[].finding_id', '$.opportunities[].criterion', '$.opportunities[].severity', '$.opportunities[].evidence_anchor', '$.opportunities[].manuscript_coordinates', '$.opportunities[].revision_operation', '$.opportunities[].preflight_status', '$.opportunities[].grounding_status'],
    ...
    dirtyDataRule: 'missing or malformed authority fields, duplicate opportunity identity, or any malformed sibling opportunity rejects RS01 persistence and reuse; governed empty opportunities remain valid',
  ...
  },
  {
    ...
    accuracyMetric: 'status is exactly: open | findings_ready | synthesis_started | proposals_ready | applied | failed | failed_retryable; allowed transitions enforced by sessionTransitions.ts',
  ...
  {
    artifact: 'revision_failure_record_v1',
    producerStageId: 'RS01_LEDGER_ASSEMBLY',
    consumerStageIds: ['RS01_LEDGER_ASSEMBLY', 'RS02_QUEUE_ADMISSION', 'RS03_QUEUE_PRIORITIZATION', 'RS04_WORKBENCH_LOAD', 'RS05_CANDIDATE_GENERATION', 'RS06_AUTHOR_DECISION', 'RS07_LEDGER_SYNC', 'RS08_COMPLETION', 'RS09_CROSSCHECK_VERIFICATION', 'RS10_TRUSTEDPATH'],
    requiredFields: ['session_id', 'stage_id', 'failure_code', 'disposition', 'retryable', 'recommended_kick', 'attempt_count', 'max_attempts', 'error_message', 'occurred_at'],
    completenessMetric: 'every non-trivial failure produces exactly one revision_failure_record_v1',
    accuracyMetric: 'disposition is exactly: retryable | terminal | manual_review; recommended_kick matches REVISE_KICK_MATRIX targetStageId',
    dirtyDataRule: 'failure without a record is invisible debt; failure record without disposition is unclassified',
    regenerationOwnerStageId: 'RS01_LEDGER_ASSEMBLY',
    requiredForAuthorExposure: false,
    fitGapStatus: 'emerging',
  },
  {
    artifact: 'candidate_hydration_failure_v1',
    producerStageId: 'RS04_WORKBENCH_LOAD',
    consumerStageIds: ['RS05_CANDIDATE_GENERATION'],
    requiredFields: ['opportunity_id', 'hydration_status', 'failure_code', 'attempt_count', 'max_attempts', 'model', 'prompt_version', 'occurred_at'],
    completenessMetric: 'every hydration failure produces a candidate_hydration_failure_v1 instead of silent blocking',
    accuracyMetric: 'hydration_status is exactly: pending | hydrated | failed_retryable | failed_terminal',
    dirtyDataRule: 'silent hydration failure creates hidden inventory; must produce failure artifact',
    regenerationOwnerStageId: 'RS04_WORKBENCH_LOAD',
    requiredForAuthorExposure: false,
    fitGapStatus: 'emerging',
  },
...
export const REVISE_FIELD_REGISTRY: readonly ReviseField[] = [
  ...
  { field: 'revision_session_status', canonicalType: 'enum', ownerArtifact: 'revision_session_v1', allowedValues: 'open | findings_ready | synthesis_started | proposals_ready | applied | failed | failed_retryable', validationRule: 'RevisionSessionStatus. Allowed transitions: open→findings_ready→synthesis_started→proposals_ready→applied|failed|failed_retryable. applied and failed are terminal. failed_retryable allows re-entry to open (requires revision_failure_record_v1). Illegal transitions throw.', fitGapStatus: 'emerging' },
...
export const REVISE_KICK_MATRIX: readonly ReviseKick[] = [
  ...
  {
    kickCode: 'REVISE_HANDOFF_RENDERER_OUTPUT_INVALID',
    triggeringStageId: 'RS02_QUEUE_ADMISSION',
    targetStageId: 'S10c_VIEWMODEL_BOUNDARY_GATE',
    triggerCondition: 'Revise admission depends on Web/PDF/DOCX/TXT renderer output or evaluation_report_view_model_v1 instead of certified Evaluation lineage and revision_opportunity_ledger_v1 provenance',
    resolution: 'Invalidate the handoff; rebuild the Revise ledger from certified Evaluation artifacts and repair the Evaluation authority boundary before admission proceeds',
    blocksAuthorExposure: false,
    severity: 'blocking',
  },
  {
  ...
  {
    kickCode: 'REVISE_ABC_NOT_DISTINCT',
    triggeringStageId: 'RS05_CANDIDATE_GENERATION',
    targetStageId: 'RS04_WORKBENCH_LOAD',
    triggerCondition: 'A/B/C candidate set fails distinctness (a pair is byte-identical or shares ≥ 0.80 content-token overlap), SHAPE (an option is empty or carries < 4 content tokens), or evidence-grounding (an option shares no content with the manuscript anchor prose or its rationale)',
    resolution: 'Regenerate candidates with materially distinct, non-empty, manuscript-grounded variants (A: Recommended, B: Rhythm Variant, C: Bolder Shift); if persistent after retry budget, terminal-block the card',
    blocksAuthorExposure: true,
    severity: 'blocking',
  },
  {
...
export const REVISE_RENDERER_CONSUMPTION_MATRIX: readonly ReviseRendererConsumer[] = [
  {
    ...
    forbiddenInputs: ['direct evaluation_result_v2 re-diagnosis', 'unbacked local-only queue item', 'fabricated readiness state', 'Web/PDF/DOCX/TXT renderer output', 'evaluation_report_view_model_v1 as revision authority'],
    mayFormatOnly: true,
  ...
  },
  {
    ...
    forbiddenInputs: ['unresolved anchor display', 'placeholder diagnostic fields', 'candidate text without source evidence', 'Web/PDF/DOCX/TXT renderer output', 'evaluation_report_view_model_v1 as revision authority'],
    mayFormatOnly: true,
    mayMutateState: false,
...
export const REVISE_CERTIFICATION_GATE_REGISTRY: readonly ReviseCertificationGate[] = [
  {
    ...
    requiredChecks: ['evaluation certified by Phase 5', 'canonical opportunity authority present and opportunities is an array', 'ledger count equals admitted disposition count', 'every opportunity has evidence anchor or manuscript-wide support', 'opportunity_id unique'],
    outputArtifact: 'revision_opportunity_ledger_v1',
    blockingFailureCodes: ['LEDGER_EVIDENCE_MISSING'],
    certificationStatus: 'partial',
    fitGapStatus: 'gap',
    notes: 'Ledger artifact exists; formal certification record is not yet emitted. A governed zero-opportunity ledger is certifiable. LEDGER_EMPTY applies only when admitted authority exists but projection unexpectedly loses every authorized opportunity.',
  },
  {
