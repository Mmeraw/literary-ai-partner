...
function resolveChunkPassConcurrency(chunkCount: number): number {
  ...
  // Adaptive default: reduce long-form pass wall-clock by limiting map-reduce waves.
...
export async function runPass2(opts: RunPass2Options): Promise<SinglePassOutput> {
  ...
  if (hasChunks) {
    ...
    if (chunksTotal > callBudget.warnTarget) {
      console.warn(
        `[Pass2] Chunk count is in warning zone (${chunksTotal} > ${callBudget.warnTarget}); ` +
    ...
    if (selectedChunks.length < chunksTotal) {
      ...
      console.warn(
        `[Pass2] Partial chunk coverage due to ${capLabel} (attempted ${selectedChunks.length}/${chunksTotal})`,
      );
    }
