...
// ─────────────────────────────────────────────────────────────────────────────
// G4 — Criteria evidence grounding (U2-004)
// ─────────────────────────────────────────────────────────────────────────────

/**
 * Per-snippet grounding result for criteria evidence validation.
 */
export interface CriteriaEvidenceSnippetResult {
  criterion_key: string;
  snippet: string;
  anchor_type: AnchorType;
  match_score: number;
...
/**
 * Grounding report for criteria[].evidence[].snippet validation.
 *
 * Parallel to EvidenceGroundingReport (which covers recommendations[].anchor_snippet).
 * Advisory-only during calibration — does NOT fail jobs.
 *
 * G4 design note:
 *   criteria[].evidence[].snippet is LLM-generated by Pass 3 with no existing
 *   grounding check. This report classifies each snippet using the same
 *   classifyAnchor logic used for recommendation anchors. When diagnosis_count > 0,
 *   a fabricated evidence snippet has reached the artifact without classification.
 *   Promotion to blocking requires live-proof that fabrication is systematic.
 *
 * Connects to G2 (pass3EvidenceFidelityCheck):
 *   grounded_count is used to populate pass3_grounded_count in EvidenceFidelityEntry,
 *   completing the third dimension of the G2 fidelity metric.
 */
export interface CriteriaEvidenceGroundingReport {
  total_snippets: number;
  verbatim_count: number;
  paraphrased_count: number;
  /** Snippets classified as editorial_diagnosis (not grounded in manuscript). */
  diagnosis_count: number;
  /** Snippets grounded in manuscript (verbatim + paraphrased). */
  grounded_count: number;
  /** Individual ungrounded snippets (for logging — not persisted). */
  ungrounded: CriteriaEvidenceSnippetResult[];
  /** True when the gate was skipped because manuscriptText was absent. */
  grounding_skipped?: boolean;
}

/**
 * Run the criteria evidence grounding gate against all criteria[].evidence[].snippet
 * in a synthesis output.
 *
 * Advisory-only. Does NOT mutate input. Does NOT fail jobs.
 * Call after Pass 3 synthesis and before Quality Gate.
 *
 * @param criteria - Array of synthesized criteria with evidence arrays
 * @param manuscriptText - The full submitted manuscript text (may be absent)
 */
export function runCriteriaEvidenceGroundingGate(
  criteria: Array<{
    key: string;
    evidence: Array<{ snippet: string }>;
  }>,
  manuscriptText: string | undefined,
): CriteriaEvidenceGroundingReport {
  const totalSnippets = criteria.reduce((sum, c) => sum + (c.evidence?.length ?? 0), 0);

  // Gate cannot run without manuscript text
  if (!manuscriptText || manuscriptText.trim().length === 0) {
    return {
  ...
  const report: CriteriaEvidenceGroundingReport = {
    total_snippets: totalSnippets,
    verbatim_count: 0,
    paraphrased_count: 0,
    diagnosis_count: 0,
  ...
  for (const criterion of criteria) {
    for (const evidenceItem of criterion.evidence ?? []) {
      const result = classifyAnchor(evidenceItem.snippet ?? "", manuscriptText);
