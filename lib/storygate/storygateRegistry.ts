...
const STORYGATE_PROCESS_REGISTRY_SOURCE: readonly StorygateProcessEntrySource[] = [
  ...
  },
  {
    ...
    codeSurfaces: ['app/storygate-studio/page.tsx', 'app/storygate-studio/apply/page.tsx', 'app/storygate-studio/faq/page.tsx'],
    processContract: 'Verify the complete agent-facing package: query letter, synopsis, author bio, elevator pitch, agent pitch, market comparables, market category, target audience, market position statement, sample pages, and rights declaration. Certified Agent Readiness output may satisfy the gate only when supplemented with every required Storygate field; equivalent professional materials may also satisfy the gate. Web/PDF/DOCX/TXT renderer output and evaluation_report_view_model_v1 are presentation surfaces, not Storygate package authority. Storygate must not treat Agent Readiness AR08 export or AR09 package history gaps as resolved. Buying RevisionGrade services is not required.',
...
export const STORYGATE_ARTIFACT_REGISTRY: readonly StorygateArtifactEntry[] = [
  { artifact: 'agent_readiness_package_v1', producerStageId: 'AGENT_READINESS_FACTORY (external)', consumerStageIds: ['SG01_CREATOR_SUBMISSION', 'SG05_PACKAGE_VERIFICATION'], requiredFields: REQUIRED_PACKAGE_FIELDS, completenessMetric: 'all required Storygate package sections present, creator-approved, and certified by Agent Readiness or equivalent professional authority', accuracyMetric: 'package reflects creator-approved materials, not inferred content', dirtyDataRule: 'Do not submit generated package content unless creator approved it for Storygate consideration. Current Agent Readiness six-section output must be supplemented with market category, target audience, market position statement, sample pages/materials, and rights declaration before Storygate package verification can pass. Storygate must not accept Web/PDF/DOCX/TXT renderer output, evaluation_report_view_model_v1, uncertified Agent Readiness output, or unresolved Agent Readiness AR08/AR09 gaps as package authority.', regenerationOwnerStageId: 'AGENT_READINESS_FACTORY (external)', requiredForControlledAccess: false, fitGapStatus: 'gap' },
...
export const STORYGATE_KICK_MATRIX: readonly StorygateKickEntry[] = [
  ...
  { kickCode: 'PACKAGE_AUTHORITY_INVALID', detectedAt: 'SG05_PACKAGE_VERIFICATION', description: 'Storygate package intake depends on renderer/download output, evaluation_report_view_model_v1, uncertified Agent Readiness output, or unresolved Agent Readiness AR08/AR09 gaps as if they were certified.', blocking: true, blocksControlledAccess: true, remediation: 'Rebuild package handoff from certified agent_readiness_package_v1 or equivalent professional materials before Storygate verification.', httpStatus: 422 },
  ...
  { kickCode: 'RIGHTS_GATE_FAILED', detectedAt: 'SG06_READINESS_VERIFICATION', description: 'Rights gate failed or rights evidence is not certified for eligibility.', blocking: true, blocksControlledAccess: true, remediation: 'Resolve rights declaration/evidence before Storygate eligibility or listing activation.', httpStatus: 422 },
  ...
  { kickCode: 'VERIFICATION_STATE_UNAUDITED', detectedAt: 'SG07_INDUSTRY_VERIFICATION', description: 'Verification state lacks required audit evidence and cannot authorize controlled-access operations.', blocking: true, blocksControlledAccess: true, remediation: 'Persist verification decision with auditable evidence before allowing access requests.', httpStatus: 500 },
  ...
  { kickCode: 'ACCESS_CONTROL_BYPASS', detectedAt: 'SG12_ACCESS_LOGGING_REVOCATION', description: 'Controlled-access checks were bypassed or grant scope was not enforced.', blocking: true, blocksControlledAccess: true, remediation: 'Invalidate current session/access path and re-enter SG10 approval + SG11 grant-scope validation.', httpStatus: 500 },
] as const;
...
export const STORYGATE_AUTHORITY_SOURCE_REGISTRY: readonly StorygateAuthoritySourceEntry[] = [
  ...
  { authorityId: 'ARTIFACT_AUTHORITY_CHAIN', family: 'doctrine', authorityLevel: 'binding', title: 'Artifact Authority Chain', path: 'docs/SIPOC_ARTIFACT_AUTHORITY_CHAIN.md', appliesToStageIds: ['SG01_CREATOR_SUBMISSION', 'SG05_PACKAGE_VERIFICATION', 'SG06_READINESS_VERIFICATION', 'SG08_LISTING_ACTIVATION'], appliesToArtifacts: ['agent_readiness_package_v1', 'package_verification_result_v1', 'storygate_eligibility_result_v1'], executionUse: 'Binding authority-order doctrine: Storygate may consume certified package artifacts but must not use ViewModel, renderer/download output, or lower-level presentation surfaces as package authority.', notes: 'Authority flows downward only; Storygate must reject renderer/VM-derived package handoffs.' },
  { authorityId: 'SIPOC_AGENT_READINESS', family: 'doctrine', authorityLevel: 'binding', title: 'Agent Readiness SIPOC/FIPOC Process Constitution', path: 'docs/SIPOC_AGENT_READINESS_PROCESS.md', appliesToStageIds: ['SG01_CREATOR_SUBMISSION', 'SG05_PACKAGE_VERIFICATION'], appliesToArtifacts: ['agent_readiness_package_v1'], executionUse: 'Defines certified Agent Readiness package handoff and unresolved AR08/AR09 gaps that Storygate must not treat as resolved.', notes: 'Storygate consumes certified Agent Readiness package output only; it does not reinterpret UED, Revise ledger, scores, recommendations, or business rules.' },
...
export const STORYGATE_CERTIFICATION_GATE_REGISTRY: readonly StorygateCertificationGateEntry[] = [
  ...
  { gateId: 'SGCG09_CERTIFIED_PACKAGE_HANDOFF_ONLY', description: 'Storygate package intake may consume certified Agent Readiness package output or equivalent professional materials only; renderer/download output, evaluation_report_view_model_v1, uncertified Agent Readiness output, and unresolved AR08/AR09 gap claims are invalid authority.', appliesToStageId: 'SG05_PACKAGE_VERIFICATION', enforced: true, testEvidence: '__tests__/lib/storygate/storygateRegistry.test.ts', notes: 'Registry/test-enforced authority boundary only; does not implement missing Storygate package verification runtime.' },
] as const;
