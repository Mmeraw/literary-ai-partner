...
describe("PR-1: Submission scope governance", () => {
  describe("classifySubmissionScope", () => {
    ...
    it("classifies chaptered 20000-49999 as multi_chapter under scope policy v2", () => {
    ...
    it("classifies 50000+ as full_manuscript", () => {
      const text = Array(60000).fill("word").join(" ");
      const profile = classifySubmissionScope(text, 1);
    ...
    it("classifies standalone 30000-word submissions as novella under scope policy v2", () => {
      const text = Array(30000).fill("word").join(" ");
      const profile = classifySubmissionScope(text, 1, "standalone");
      expect(profile.inputScale).toBe("novella");
      expect(profile.confidenceCapSummary).toBe("HIGH");
    });
  });
