  ...
  return {
    ...
    canonicalOpportunityLedger: {
      ...
      metrics: {
        ...
        quality_gate: {
          repeated_final_issue_ratio: 0,
          repaired_duplicate_rendering: true,
...
describe('REVISION_SURFACE_OWNERSHIP_GATE', () => {
  describe('runRevisionSurfaceOwnershipGate', () => {
    ...
    it('accepts governed empty canonical authority and rejects missing or malformed authority', () => {
      const governedEmpty = runRevisionSurfaceOwnershipGate(makeMinimalUED({
        revisionOpportunitySummary: { total: 0, high: 0, medium: 0, low: 0 },
        criterionDetails: [],
        topRecommendations: [],
      }));
      expect(governedEmpty.status).toBe('pass');

      const missing = makeMinimalUED();
      delete (missing as Record<string, unknown>).canonicalOpportunityLedger;
      expect(runRevisionSurfaceOwnershipGate(missing)).toMatchObject({
        status: 'fail',
        failures: [expect.objectContaining({ failure_code: 'CANONICAL_OPPORTUNITY_AUTHORITY_INVALID' })],
      });

      const malformed = makeMinimalUED();
      (malformed as Record<string, unknown>).canonicalOpportunityLedger = { opportunities: {} };
      expect(runRevisionSurfaceOwnershipGate(malformed)).toMatchObject({
        status: 'fail',
        failures: [expect.objectContaining({ failure_code: 'CANONICAL_OPPORTUNITY_AUTHORITY_INVALID' })],
      });
    });
