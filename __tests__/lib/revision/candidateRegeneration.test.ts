/**
 * Unit tests for candidateRegeneration.ts
 *
 * Tests the regeneration loop: heals quality-failed cards via a second
 * hydration pass, and fails closed when the second attempt also fails quality.
 */
...
// Prose that passes all quality checks
...
describe('regenerateCandidatesForQualityFailed', () => {
  beforeEach(() => {
  ...
  it('fails closed when hydration succeeds but prose still fails quality', async () => {
    ...
    // Return prose that will fail the quality gate (generic advice)
    mockHydrate.mockResolvedValueOnce({
      hydratedCount: 1,
      skippedCount: 0,
      candidates: new Map([
    ...
    const reasons = result.stillFailed.get('rol:still-bad')!;
    expect(reasons).toContain('candidate_quality_failed_after_regen');
  });
  ...
  it('fails closed when hydration returns no candidates for the opportunity', async () => {
    ...
    mockHydrate.mockResolvedValueOnce({
      hydratedCount: 0,
      skippedCount: 0,
      candidates: new Map(), // ID not returned
    });
  ...
  it('handles mixed batch: some heal, some remain failed', async () => {
    ...
    mockHydrate.mockResolvedValueOnce({
      ...
      candidates: new Map([
        ...
        // rol:fail-b gets generic advice that fails quality
        [
          'rol:fail-b',
          {
  ...
  it('calls hydrateLedgerCandidates exactly once (MAX_REGEN_ATTEMPTS=1)', async () => {
