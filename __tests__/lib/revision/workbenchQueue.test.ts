...
describe('getWorkbenchQueue', () => {
  ...
  it('fails closed when the persisted ledger contains duplicate opportunity IDs', async () => {
    ...
    mockEnsureLedger.mockResolvedValueOnce({
      ...
      opportunities: [
        makeLedgerOpportunity({ opportunity_id: 'duplicate-opportunity' }),
        makeLedgerOpportunity({
          opportunity_id: 'duplicate-opportunity',
          criterion: 'VOICE_CONTROL',
        }),
      ] as never,
    });
    ...
    expect(result.ok).toBe(false);
    expect(result.error).toContain('duplicate opportunity id');
    expect(result.error).toContain('duplicate-opportunity');
    expect(result.opportunities).toHaveLength(0);
    expect(result.needsTargeting).toHaveLength(0);
