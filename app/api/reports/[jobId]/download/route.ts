...
// PDFKit removed — using Chromium-based PDF rendering via buildChromiumPdf
...
async function buildChromiumPdf(html: string): Promise<Buffer> {
  ...
  const browser = await puppeteer.launch({
    // @ts-expect-error — chromium.args type narrowing
    args: chromium.args,
    // @ts-expect-error — chromium.defaultViewport type
    defaultViewport: chromium.defaultViewport,
    // @ts-expect-error — chromium.executablePath return type
    executablePath: await chromium.executablePath(),
    // @ts-expect-error — chromium.headless type
    headless: chromium.headless ?? true,
  });

  try {
    const page = await browser.newPage();
    // @ts-expect-error — puppeteer-core waitUntil type mismatch with installed types
