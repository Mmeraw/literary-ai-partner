...
export default function ReviseCockpitClientChoiceLedgerV2({ payload }: { payload: WorkbenchQueuePayload }) {
      ...
      const response = await fetch("/api/revision-ledger", {
        ...
        body: JSON.stringify({
          ...
          entries: [{
            ...
            isUndo: false,
            undoneLocalId: null,
            metadata: { source: "choice-highlight-revise-cockpit-v2", revisionOperation: effectiveOperation(item), criterion: criterionOf(item) },
          }],
        }),
      });
