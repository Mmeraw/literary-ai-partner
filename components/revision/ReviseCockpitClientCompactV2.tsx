...
function fallbackCandidate(item: WorkbenchOpportunity, key: OptionKey): string {
  const haystack = compareText(`${item.title} ${item.issueStatement} ${sourceTextOf(item)}`);
  const insertion = effectiveOperation(item) === "insert_after_selected_passage" || effectiveOperation(item) === "insert_before_selected_passage";
  if (!insertion) return "";
...
export default function ReviseCockpitClientCompactV2({ payload }: { payload: WorkbenchQueuePayload }) {
    ...
    try {
      const response = await fetch("/api/revision-ledger", {
        ...
        body: JSON.stringify({
          ...
          entries: [{ localId: entry.id, opportunityId: item.id, opportunityTitle: item.title, decision: entry.decision, selectedOption: entry.option ?? null, selectedText: selectedText ?? null, customText: null, sourceExcerpt: sourceTextOf(item) || null, sourceLocation: item.anchor || item.meta || null, clientCreatedAt: new Date().toISOString(), isUndo: false, undoneLocalId: null, metadata: { source: "spacious-revise-cockpit", revisionOperation: effectiveOperation(item), criterion: criterionOf(item) } }],
        }),
      });
  ...
  function decide(decision: DecisionState, option?: OptionKey, text?: string) {
    ...
    const entry: LedgerEntry = { id: `local-${Date.now()}-${Math.random().toString(36).slice(2)}`, itemId: active.id, itemTitle: active.title, decision, option, criterion: criterionOf(active), syncStatus: "pending" };
  ...
  if (!payload.ok || items.length === 0) {
    return <main className="fixed inset-x-0 bottom-0 top-[72px] flex items-center justify-center bg-[#0D0A05] px-3 pb-4 pt-2 text-[#F5EFE4]">No revision queue available.</main>;
  }

  return (
    <main className="fixed inset-x-0 bottom-0 top-[72px] z-10 overflow-hidden bg-[#0D0A05] px-3 pb-4 pt-2 text-[#F5EFE4]">
      <div className="mx-auto flex h-full max-w-[calc(100vw-1.5rem)] flex-col overflow-hidden rounded-xl border border-[#2E261A] bg-[#151008] shadow-2xl">
