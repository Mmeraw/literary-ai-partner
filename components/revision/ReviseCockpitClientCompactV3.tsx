...
function effectiveOperation(item: WorkbenchOpportunity): RevisionOperation {
  ...
  const haystack = [item.title, item.issueStatement, item.fixDirection, item.diagnostic?.operationTargeting, sourceTextOf(item)].map(normalize).join(" ").toLowerCase();
...
function genericFallbackCandidate(_item: WorkbenchOpportunity, _key: OptionKey): string {
  // Return empty — generic meta-descriptions are not copy-paste-ready manuscript prose.
  // The candidate will show as needing targeting with real manuscript text.
  return "";
}
...
export default function ReviseCockpitClientCompactV3({ payload }: { payload: WorkbenchQueuePayload }) {
  ...
  return (
    <main className="fixed inset-x-0 bottom-0 top-[72px] z-10 overflow-hidden bg-[#0D0A05] px-4 pb-5 pt-3 text-[#F5EFE4]">
      <div className="mx-auto flex h-full max-w-[calc(100vw-2rem)] flex-col overflow-hidden rounded-xl border border-[#2E261A] bg-[#151008] shadow-2xl">
        <header className="flex shrink-0 items-center justify-between border-b border-[#2E261A] bg-[#151008] px-4 py-4">
          <div className="min-w-0"><p className="text-[10px] uppercase tracking-[0.2em] text-[#C8A96E]">Revision Cockpit · surgical repair queue</p><h1 className="truncate text-sm font-semibold pb-2" style={{ lineHeight: '2' }}>{payload.manuscriptTitle}</h1></div>
          {message && <div className="flex gap-2 text-[11px]"><span className="rounded border border-[#5D4C31] px-2 py-1 text-[#A9987D]">{message}</span></div>}
        </header>
        ...
        <section className="grid min-h-0 flex-1 overflow-hidden xl:grid-cols-[310px_minmax(0,1fr)]">
          <aside className="flex min-h-0 flex-col border-r border-[#2E261A] bg-[#110D07]">
            <div className="space-y-2 border-b border-[#2E261A] p-2"><div style={{display:'flex',flexDirection:'row',flexWrap:'nowrap',alignItems:'center',justifyContent:'space-between',gap:'8px'}}><p style={{flexShrink:0,whiteSpace:'nowrap',fontSize:'11px',textTransform:'uppercase',letterSpacing:'0.1em',color:'#C8A96E',margin:0}}>Search Queue</p><span style={{flexShrink:0,whiteSpace:'nowrap',borderRadius:'4px',border:'1px solid rgba(200,169,110,0.7)',background:'rgba(200,169,110,0.1)',padding:'4px 8px',fontSize:'11px',fontWeight:600,color:'#F3E3C3'}}>{"Queue\u2003"}{filtered.length ? activeIndex + 1 : 0}/{filtered.length}</span></div><input value={filters.search} onChange={(event) => setFilters((current) => ({ ...current, search: event.target.value }))} placeholder="Search queue" className="h-8 w-full rounded border border-[#3A3022] bg-[#0D0A05] px-2 text-xs" /><div className="grid grid-cols-2 gap-2"><select value={filters.priority} onChange={(event) => setFilters((current) => ({ ...current, priority: event.target.value as Filters["priority"] }))} className="h-8 rounded border border-[#3A3022] bg-[#0D0A05] px-2 text-xs"><option value="all">All priority</option><option value="must">MUST</option><option value="should">SHOULD</option><option value="could">COULD</option></select><select value={filters.status} onChange={(event) => setFilters((current) => ({ ...current, status: event.target.value as DecisionFilter }))} className="h-8 rounded border border-[#3A3022] bg-[#0D0A05] px-2 text-xs"><option value="all">All status</option><option value="pending">Pending</option><option value="accepted">Accepted</option><option value="custom">Custom</option><option value="kept_original">Kept</option><option value="rejected">Rejected</option><option value="deferred">Deferred</option></select></div><select value={filters.criterion} onChange={(event) => setFilters((current) => ({ ...current, criterion: event.target.value }))} className="h-8 w-full rounded border border-[#3A3022] bg-[#0D0A05] px-2 text-xs"><option value="all">All criteria</option>{criteria.map((criterion) => <option key={criterion} value={criterion}>{criterion}</option>)}</select></div>
            <ol className="min-h-0 flex-1 space-y-1 overflow-y-auto p-2">{filtered.map((item, index) => <li key={item.id}><button type="button" onClick={() => selectItem(item.id)} className={`w-full rounded-lg border p-2 text-left ${item.id === active?.id ? "border-[#C8A96E] bg-[#221B11]" : "border-[#2B241A] bg-[#161109]"}`}><div className="mb-1 flex flex-wrap gap-1"><span className={`rounded px-1.5 py-0.5 text-[10px] uppercase ${severityClass(item.severity)}`}>{item.severity}</span><span className="rounded border border-[#4E4333] px-1.5 py-0.5 text-[10px]">{item.scope}</span><span className="rounded border border-[#4E4333] px-1.5 py-0.5 text-[10px] text-[#A9987D]">{operationLabels[effectiveOperation(item)] ?? "Revision"}</span><span className="rounded border border-[#4E4333] px-1.5 py-0.5 text-[10px] text-[#A9987D]">{decisionGroup(decisionById[item.id])}</span></div><p className="line-clamp-2 text-xs leading-4">{index + 1}. {item.title}</p><p className="mt-1 truncate text-[11px] text-[#A9987D]">{criterionOf(item)} · {item.anchor || item.meta}</p></button></li>)}</ol>
          </aside>

          <section className="flex min-w-0 flex-col overflow-hidden bg-[#1C160E]">
            {active && <>
              <div className="shrink-0 border-b border-[#2E261A] px-3 py-0.5">
                <p className="text-[9px] uppercase tracking-[0.18em] text-[#C8A96E]">Diagnosis & Guardrails</p>
                <div className="flex flex-wrap items-center gap-1"><span className={`shrink-0 rounded px-2 py-0.5 text-[10px] uppercase ${severityClass(active.severity)}`}>{active.severity}</span><span className="shrink-0 rounded border border-[#5A4B33] px-2 py-0.5 text-[10px] uppercase tracking-wider">{criterionOf(active)}</span><span className="shrink-0 rounded border border-[#5A4B33] px-2 py-0.5 text-[10px] uppercase tracking-wider">{active.scope}</span><span className="shrink-0 rounded border border-[#5A4B33] px-2 py-0.5 text-[10px] uppercase tracking-wider">{operationLabels[effectiveOperation(active)] ?? "Revision"}</span></div>
              </div>
              <div className="shrink-0 border-b border-[#2E261A] px-3 pb-2 pt-10"><h2 className="text-base font-semibold" style={{ lineHeight: '1.5' }}>{active.title}</h2></div>

              <div className="min-h-0 flex-1 overflow-y-auto p-3">
                <section className="rounded-xl border border-[#2E261A] bg-[#12100B] p-3"><div className="grid gap-x-4 gap-y-1 text-sm leading-5 xl:grid-cols-2"><p><span className="text-[#C8A96E]">Symptom:</span> {diagnosticText(active, "symptom")}</p><p><span className="text-[#C8A96E]">Cause:</span> {diagnosticText(active, "cause")}</p><p><span className="text-[#C8A96E]">Fix:</span> {diagnosticText(active, "fix")}</p><p><span className="text-[#C8A96E]">Reader effect:</span> {diagnosticText(active, "readerEffect")}</p><p><span className="text-[#C8A96E]">Mistake-proofing:</span> {diagnosticText(active, "mistakeProofing")}</p><p><span className="text-[#C8A96E]">Operation:</span> {operationLabels[effectiveOperation(active)]}</p></div></section>
              ...
              <section className="shrink-0 border-t border-[#2E261A] bg-[#0D0A05] px-3 pb-3 pt-3">{ledger.length === 0 ? <div className="flex gap-3 rounded border border-[#2E261A] bg-[#120E08] px-3 py-2 text-xs"><span className="uppercase tracking-[0.16em] text-[#C8A96E]">Revision Ledger</span><span className="text-[#A9987D]">No decisions yet.</span></div> : <><div className="mb-2 flex flex-wrap gap-2 text-xs"><span className="mr-2 uppercase tracking-[0.16em] text-[#C8A96E]">Revision Ledger</span>{LEDGER_FILTERS.map((filter) => <button key={filter.value} type="button" onClick={() => setFilters((current) => ({ ...current, status: filter.value }))} className={`rounded border px-2 py-1 ${filters.status === filter.value ? "border-[#C8A96E]" : "border-[#3A3022] text-[#A9987D]"}`}>{filter.label}</button>)}</div><div className="max-h-24 overflow-y-auto rounded border border-[#2E261A]"><table className="w-full text-left text-xs"><thead className="sticky top-0 bg-[#161109] text-[#C8A96E]"><tr><th className="px-2 py-1">Decision</th><th className="px-2 py-1">Option</th><th className="px-2 py-1">Criterion</th><th className="px-2 py-1">Opportunity</th><th className="px-2 py-1">Sync</th></tr></thead><tbody>{visibleLedger.map((entry) => <tr key={entry.id} className="border-t border-[#2E261A]"><td className="px-2 py-1">{decisionLabel(entry.decision)}</td><td className="px-2 py-1">{entry.option ?? "—"}</td><td className="px-2 py-1">{entry.criterion}</td><td className="max-w-[520px] truncate px-2 py-1">{entry.itemTitle}</td><td className="px-2 py-1">{entry.syncStatus}</td></tr>)}</tbody></table></div></>}</section>
            </>}
          </section>
        </section>
      </div>
