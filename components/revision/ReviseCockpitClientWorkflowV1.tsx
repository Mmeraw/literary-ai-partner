...
type SourceFilter = "all" | "craft" | "surface_polish";
...
export default function ReviseCockpitClientWorkflowV1({ payload }: { payload: WorkbenchQueuePayload }) {
  ...
  const filtered = useMemo(() => {
    ...
    return openItems.filter((item) => {
      if (filters.sourceFilter === "craft" && item.source === "surface_polish") return false;
  ...
  return (
    <main className="fixed inset-x-0 bottom-0 top-[72px] z-10 overflow-hidden bg-[#0D0A05] px-4 pb-5 pt-3 text-[#F5EFE4]">
      <div className="mx-auto flex h-full max-w-[calc(100vw-2rem)] flex-col overflow-hidden rounded-xl border border-[#2E261A] bg-[#151008] shadow-2xl">
        ...
        <nav className="flex shrink-0 items-center gap-1 border-b border-[#2E261A] bg-[#110D07] px-4 py-1.5">{([["all", "All"], ["craft", "Craft"], ["surface_polish", "Surface Polish"]] as const).map(([value, label]) => <button key={value} type="button" onClick={() => setFilters((current) => ({ ...current, sourceFilter: value }))} className={`rounded px-2.5 py-1 text-[11px] font-medium transition-colors ${filters.sourceFilter === value ? "bg-[#C8A96E]/20 text-[#F5EFE4] border border-[#C8A96E]" : "text-[#A9987D] hover:text-[#F5EFE4] border border-transparent"}`}>{label}{value === "surface_polish" && <span className="ml-1 rounded bg-[#B8922A]/20 px-1 py-0.5 text-[9px] text-[#C8A96E]">Polish</span>}</button>)}</nav>
