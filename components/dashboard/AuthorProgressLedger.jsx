...
const INTERNAL_READY_THRESHOLD = 9
const PUBLIC_READY_THRESHOLD = 90
const PUBLIC_SCORE_MAX = 100

const toPublicScore = (value) => (typeof value === 'number' ? value * 10 : value)
const fmt = (n) => formatScoreForDisplay(toPublicScore(n))
const signedFmt = (n) => formatSignedScoreForDisplay(toPublicScore(n))
...
function countAgentReady(rows) {
  const latestPerManuscript = new Map()
  for (const row of rows) {
    const existing = latestPerManuscript.get(row.manuscriptId)
    if (!existing || new Date(row.createdAt).getTime() > new Date(existing.createdAt).getTime()) {
      latestPerManuscript.set(row.manuscriptId, row)
    }
  }

  let count = 0
  for (const row of latestPerManuscript.values()) {
    const top = Math.max(row.overallScore ?? 0, row.readinessScore ?? 0)
    if (top >= INTERNAL_READY_THRESHOLD) count += 1
  }
  return count
}

function normalizeReadinessStatus(row) {
  if (!['market_ready', 'near_ready', 'improving', 'below_standard', 'complete'].includes(row.status)) {
    return row
  }

  const top = Math.max(row.overallScore ?? 0, row.readinessScore ?? 0)
  const status = top >= INTERNAL_READY_THRESHOLD
    ? 'market_ready'
    : top >= 6
      ? 'near_ready'
      : 'below_standard'
...
export default function AuthorProgressLedger({ rows, kpis, reviseAnalytics }) {
  const normalizedRows = rows.map(normalizeReadinessStatus)
  const latest = normalizedRows[0]
  const prior = normalizedRows.find((r) => r.manuscriptId === latest.manuscriptId && r.id !== latest.id)
  ...
  const points = trend(normalizedRows, latest)
  const canPackage = latest.status !== 'running' && latest.status !== 'failed'
  const agentReadyCount = countAgentReady(normalizedRows)
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {[
          ['Latest overall', fmt(latest.overallScore), overallDelta == null ? 'Awaiting comparison' : `${signedFmt(overallDelta)} vs prior`],
          ['Agent readiness', fmt(latest.readinessScore), readinessDelta == null ? `Threshold is ${PUBLIC_READY_THRESHOLD}/${PUBLIC_SCORE_MAX}` : `${signedFmt(readinessDelta)} readiness delta`],
          ['Revise decisions', String(revise.uniqueDecidedOpportunities), `${revise.priorities.pending} opportunities pending`],
          ['Agent ready', String(agentReadyCount), `Latest works at or above ${PUBLIC_READY_THRESHOLD}/${PUBLIC_SCORE_MAX}`],
        ...
        <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
          ...
          <div className="space-y-4">
            {[
              ...
              [`To ${PUBLIC_READY_THRESHOLD}/${PUBLIC_SCORE_MAX}`, topScore, topScore >= INTERNAL_READY_THRESHOLD ? 'Met' : `${formatScoreForDisplay((INTERNAL_READY_THRESHOLD - topScore) * 10)} points left`],
            ].map(([label, value, text]) => (
              <div key={label} className="grid grid-cols-[90px_1fr_90px] items-center gap-3">
      ...
      <EvaluationHistoryTable rows={normalizedRows} />
      ...
      <section className="grid gap-6 xl:grid-cols-2">
        <article className="rounded-3xl border border-neutral-300 bg-white p-6 text-neutral-950">
          ...
          <div className="relative mt-6 flex h-64 items-end gap-4 border-b border-l border-neutral-200 px-4 pb-8">
            <div className="absolute left-0 right-0 top-[10%] border-t border-dashed border-amber-500"><span className="absolute -top-3 left-3 bg-white px-2 text-xs font-bold text-amber-700">{PUBLIC_READY_THRESHOLD}/{PUBLIC_SCORE_MAX} threshold</span></div>
