...
- Worker route runtime budget: `app/api/workers/process-evaluations/route.ts` exports `maxDuration = 900`.
- Per-pass timeout cap: `EVAL_PASS_TIMEOUT_MS` is clamped to `<= 900000` ms in `lib/evaluation/processor.ts`.
- OpenAI timeout cap: `EVAL_OPENAI_TIMEOUT_MS` is clamped to `<= 900000` ms in processor and pass runners.
