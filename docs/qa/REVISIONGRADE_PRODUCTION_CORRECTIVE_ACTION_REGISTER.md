...
9. Superseded executable authority is deleted in the same corrective unit. Retention requires production-row evidence, a named read-only compatibility boundary, an owner, a sunset condition, and a regression test proving it cannot create current authority.
10. A path described as historical or legacy must be unreachable from new submissions and current writes. If it can still route, synthesize, certify, persist, or project new data, it is active authority and must be governed or purged.
11. Every purge adds a merge-blocking architecture invariant that rejects reintroduction of the removed authority and proves all remaining production entry points use the canonical replacement.
...
| REL-E007 | Calibration tooling + historical evidence triage | PR #1365 / `2fc459bf178267a0603bafffbf71f4e1c215980f` | PG-06B analyzer, privacy-safe output tests, and first committed historical evidence pass are merged. Four historical files showed 24 weak zero-recommendation criteria with missing governed disposition metadata, supporting historical disposition/ownership loss rather than editorial under-generation. This does not close the representative post-#1362 editorial calibration study. |
...
| RCA-002 | Explain all recommendation dispositions without converting lineage into queue quota | Contract reconciliation | Observed production + CI verification + static contract | Confirmed Pass 2→Pass 3 source-loss escape; correction pending publication/deployment | Editorial governance / Pass 3 owner | Correcting | Trace every meaningful Pass 2 source ID through deterministic materialization, named consolidation, or governed suppression before normalization/persistence; inspect canonical ledger and Revise projection after deployment | Missing, duplicate, unknown, orphan materialized/consolidated, or governance-free suppression fails at S07 and uses only the registered one-time Pass 3 kick; exhausted or mixed defects cannot persist or project to Revise | Governed disposition v1; stable source identity; S07 retry contract; representative calibration corpus | Current writer rejects an unaccounted Pass 2 source; lineage persists as non-author-facing evaluation transparency; canonical ledger/Revise evidence reconciles every source without a score/count admission predicate or synthetic Held promotion | Diamonds Aren't Forever `7c3f47a6-bfc2-4025-a7fc-d1ce0456a609`; PR #1354; PR #1358 / REL-E001; PR #1362 / REL-E005; pending lineage correction |
...
| RCA-005 | Determine whether zero actionable opportunities on strong-but-imperfect evaluations is governed suppression or editorial loss | Forensic investigation | Observed production + CI verification + static contract | Historical propagation diagnosis strong; Diamonds source-accounting loss now a separate confirmed RCA-002 path; current editorial calibration still unproven | Editorial governance | Characterizing | First close deterministic Pass 2→Pass 3→ledger lineage for the Diamonds case; then run the stratified post-correction calibration review and blind editorial adjudication | Unsafe or ungrounded recommendations remain suppressed; investigation cannot promote work solely to satisfy a count or score expectation | RCA-002 source-lineage correction; representative manuscript corpus; adjudication rubric; PG-06B analyzer from REL-E007 | At least 30 consented/redacted post-correction evaluations across score bands, genre strata, and short-/long-form length classes classify each reviewed case as correct governed suppression, genuine editorial omission, unsupported recommendation pressure, grounding failure, status/cardinality defect, ownership loss, canonicalization loss, or unsupported threshold hypothesis. Correction follows only proven defects. | Criminality V2; Diamonds Aren't Forever `7c3f47a6-bfc2-4025-a7fc-d1ce0456a609`; historical 82/100 zero-card run; PR #1362 / REL-E005; PR #1365 / REL-E007 |
...
| RCA-013 | Purge superseded active authorities and isolate only proven historical readers | Reliability hardening | Static contract + code census | Strong for identified paths; production-row census pending | Evaluation architecture / release engineering | Correcting | Executable import/caller scan; production row/version census; current-write reachability tests; architecture-invariant guard | A compatibility path may read only a proven historical shape; it cannot route a new job, synthesize current prose, certify, persist, project, or silently repair current authority | Recommendation-disposition purge; production artifact census; long-form routing inventory | Recommendation-disposition fallbacks and raw cache resurrection removed; remaining ECG bypass modes, generated-prose fallbacks, historical artifact resume paths, single-model degradation naming, long-form routing authority, and dead processor normalizers each become either governed current behavior or deleted code; every retained reader has evidence, owner, sunset, and no-current-write proof | PR #1362 worktree audit; architecture-invariants test; forthcoming authority-purge evidence |
