...
A long-form multi-layer evaluation is the deepest RevisionGrade evaluation mode. It diagnoses a manuscript at manuscript scale while preserving architecture that would be flattened by a standard long-form report: multiverse, multi-universe, multi-world, human-animal-nature-earth-the elements-God-normally inanimate objects that are rendered as characters, multiple timelines, voice lanes, symbolic systems, canon/doctrine layers, identity systems, research-heavy ambiguity, paratext, documentary structures, nested narratives, or other layered structures.
...
| Scene Construction & Function | XX/10 | Low |
...
| Tonal Authority & Consistency | XX/10 | Low |
...
Reading Grade Level is Title Block metadata only for long-form multi-layer evaluation unless a future template revision explicitly authorizes a separate explanatory section.
...
Dialogue/Narrative Ratio is Title Block metadata only for long-form multi-layer evaluation unless a future template revision explicitly authorizes a separate explanatory section.
...
The submission is too fragmented, incomplete, inconsistent, degraded, or context-limited to support strong conclusions. Findings should be treated as provisional.

Confidence must not be inflated because a report completed successfully. Confidence reflects evidence quality and diagnostic reliability, not pipeline completion.
