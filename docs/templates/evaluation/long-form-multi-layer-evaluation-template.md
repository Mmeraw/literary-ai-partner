...
This template clarifies that those DREAM surfaces belong to the long-form multi-layer product mode when a manuscript requires them.
...
- avoid verbatim duplication of criterion opportunities;
...
WAVE must not be presented as the revision workflow. Revise Queue and TrustedPath are the repair workflows.
