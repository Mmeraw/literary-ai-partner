...
|---|---|---|---|---|---|---|
| *Froggin Noggin* | [`froggin-noggin-dream-longform-multilayer-gold-standard.md`](./froggin-noggin-dream-longform-multilayer-gold-standard.md) | Folded into unified one-file benchmark; legacy addendum removed from active folder. | `long_form_multi_layer_evaluation` | `multi_layer_long_form` | Required gold | Unified required-gold benchmark path is the only active Froggin Noggin benchmark authority. |
...
Do not weaken the underlying evaluation body while normalizing headers. Header normalization is metadata only. Addenda or unified canonical files are the preferred V2 compliance layer for large manual benchmark bodies.
