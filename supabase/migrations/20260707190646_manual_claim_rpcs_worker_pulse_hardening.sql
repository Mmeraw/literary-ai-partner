-- =============================================================================
-- Migration: Manual claim RPC hardening
-- Date: 2026-07-07
-- Origin: Applied directly to production via Supabase dashboard (no PR).
--         Recorded here for migration parity only.
--
-- Changes:
--   - claim_evaluation_jobs: enforces production:<ip>:<traceId> worker_id
--     pattern; sets worker_pulse_at at claim time to prevent null-pulse orphan
--     false positives; respects next_attempt_at backoff; FOR UPDATE SKIP LOCKED.
--   - claim_evaluation_job_by_id: same pattern enforcement + worker_pulse_at.
-- =============================================================================
