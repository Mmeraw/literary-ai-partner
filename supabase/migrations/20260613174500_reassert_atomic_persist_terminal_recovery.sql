-- =============================================================================
-- Migration: Reassert atomic persist terminal recovery
-- Date: 2026-06-13
-- Purpose:
--   Production regression for job 38680759-8c80-4d16-b61f-f198c2d1eb67 showed
--   persist_evaluation_v2_atomic still attempting failed -> complete directly in
--   at least one environment. Re-creating the function under a fresh migration
--   guarantees the deployed DB has the terminal recovery path:
--
--     failed/degraded/cancelled -> queued -> running -> complete
--
--   This is the only legal completion path when a watchdog/stale worker has
--   already marked phase_status terminal while a healthy worker reaches artifact
--   persistence.
--
-- Governance:
