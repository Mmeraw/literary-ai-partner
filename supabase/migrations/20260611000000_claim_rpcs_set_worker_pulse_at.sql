-- Migration: claim_rpcs_set_worker_pulse_at
-- Purpose:
--   Fix split-brain double-claim race condition. The watchdog's null-pulse orphan
--   sweep (rescueNullPulseOrphans) re-queues any running job where worker_pulse_at
--   IS NULL and updated_at is older than 30 seconds. If the processor takes >30s
--   before its first pulseWorker() call (e.g. during manuscript fetch), the watchdog
--   treats it as an orphan and re-queues while the first worker is still active.
--
--   Fix: Set worker_pulse_at = now() at claim time so newly claimed jobs are never
--   visible to the null-pulse orphan sweep.
--
--   Incident: Job 71d2d1b6 — 5 claim events on same job, ARTIFACT_PERSISTENCE_FAILED
--   due to concurrent workers racing through phases.
...
COMMENT ON FUNCTION public.claim_evaluation_jobs(integer, text, uuid, timestamp with time zone) IS
  'Batch claim RPC. Sets worker_pulse_at at claim time to prevent null-pulse orphan false positives. Enforces production:<ip>:<traceId> worker_id pattern. Respects next_attempt_at backoff. FOR UPDATE SKIP LOCKED prevents double-claim.';
...
COMMENT ON FUNCTION public.claim_evaluation_job_by_id(uuid, text, uuid, timestamp with time zone) IS
  'Single-job claim RPC. Sets worker_pulse_at at claim time to prevent null-pulse orphan false positives. Enforces production:<ip>:<traceId> worker_id pattern. Respects next_attempt_at backoff.';
