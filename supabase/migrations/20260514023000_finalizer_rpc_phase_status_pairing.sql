-- Migration: Finalizer RPC top-level phase_status pairing fix
-- Purpose: Re-declare finalizer_complete_job_atomic and finalizer_mark_job_failed
--          to write top-level evaluation_jobs.phase_status alongside status,
--          satisfying the enforce_evaluation_jobs_status_phase_consistent trigger.
-- Context: Closes a P0001 rollback path discovered while triaging the
--          Froggin Noggin (job d7c77f83…) hang at 84%. The prior finalizer
--          RPCs (20260405000000, 20260405000001) only wrote phase_status inside
--          the progress JSONB column, which does not satisfy the trigger
--          requirement of paired top-level status/phase_status columns.
