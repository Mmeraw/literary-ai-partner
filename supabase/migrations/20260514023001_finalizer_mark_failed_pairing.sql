-- Migration: Finalizer RPC top-level phase_status pairing fix (part 2/2)
-- Purpose: Re-declare finalizer_mark_job_failed to write top-level
--          evaluation_jobs.phase_status alongside status, satisfying the
--          enforce_evaluation_jobs_status_phase_consistent trigger.
-- Context: Closes the failure-path counterpart of the rollback hazard
--          addressed in 20260514023000_finalizer_complete_pairing.sql.
...
-- Split:   Single CREATE OR REPLACE FUNCTION per file is required because
--          Supabase's migration runner uses simple-query and rejects multi-
--          command prepared statements (SQLSTATE 42601). See part 1/2 for
--          the complete-job RPC.
