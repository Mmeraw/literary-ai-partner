...
-- Ensures exactly-once job execution via database-level locking
...
DECLARE
  v_job_id UUID;
BEGIN
  -- Find and lock next eligible job atomically
  ...
  LIMIT 1
  FOR UPDATE SKIP LOCKED; -- Critical: prevents concurrent claims
