...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='evaluation_jobs' AND column_name='evaluation_result') THEN
    EXECUTE 'COMMENT ON COLUMN public.evaluation_jobs.evaluation_result IS ''EvaluationResultV1 JSON payload (validated in application layer). See schemas/evaluation-result-v1.ts for structure.''';
  END IF;
END $$;
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='evaluation_jobs' AND column_name='evaluation_result_version') THEN
    EXECUTE 'COMMENT ON COLUMN public.evaluation_jobs.evaluation_result_version IS ''Schema version (e.g., "evaluation_result_v1"). Used to handle schema evolution.''';
  END IF;
END $$;
